...
const Select = styled.select`
  padding: ${({ theme }) => `${theme.gridUnit * 1.5}px ${theme.gridUnit * 3}px`};
...
const NoData = styled.div`
  ...
  padding: ${({ theme }) => theme.gridUnit * 15}px ${({ theme }) => theme.gridUnit * 5}px;
  color: ${({ theme }) => theme.colors.grayscale.base};
  text-align: center;
`;
...
const SM24CustomerProductUsageViz: FC<SM24CustomerProductUsageVizProps> =
  props => {
    const {
      width,
      height,
    ...
    }, [filteredTrends, chartType, showDauLine, showSessionsBars, formatDate, theme]);
      ...
      <Container width={width} height={height}>
        ...
        >
          ...
          <div>
            ...
            <MetricValue>
              {formatPercent(data.overallAdoptionScore)}
            </MetricValue>
          </div>
          <div>
